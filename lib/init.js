...
function generateEnv(answers) {
  ...
  if (
    ...
    envLines.push(`GITHUB_TOKEN=${answers.githubRepoToken}`);
  }
  if (
    answers.customRegistryToken &&
    answers.customRegistryToken.toLowerCase() !== 'na'
  ) {
    envLines.push(`CUSTOM_REGISTRY_TOKEN=${answers.customRegistryToken}`);
  }
  if (
    answers.artifactoryToken &&
    answers.artifactoryToken.toLowerCase() !== 'na'
  ) {
    envLines.push(`ARTIFACTORY_TOKEN=${answers.artifactoryToken}`);
  }
  if (answers.nexusToken && answers.nexusToken.toLowerCase() !== 'na') {
    envLines.push(`NEXUS_TOKEN=${answers.nexusToken}`);
  }
  if (answers.verdaccioToken && answers.verdaccioToken.toLowerCase() !== 'na') {
    envLines.push(`VERDACCIO_TOKEN=${answers.verdaccioToken}`);
  }
  if (
    answers.customRegistryUrl &&
    answers.customRegistryUrl.toLowerCase() !== 'na'
  ) {
    envLines.push(`CUSTOM_REGISTRY_URL=${answers.customRegistryUrl}`);
  }
  if (answers.artifactoryUrl && answers.artifactoryUrl.toLowerCase() !== 'na') {
    envLines.push(`ARTIFACTORY_REGISTRY_URL=${answers.artifactoryUrl}`);
  }
  if (answers.nexusUrl && answers.nexusUrl.toLowerCase() !== 'na') {
    envLines.push(`NEXUS_REGISTRY_URL=${answers.nexusUrl}`);
  }
  if (answers.verdaccioUrl && answers.verdaccioUrl.toLowerCase() !== 'na') {
    envLines.push(`VERDACCIO_REGISTRY_URL=${answers.verdaccioUrl}`);
  }
...
async function init(
  ...
  try {
    ...
    const isGitHub = ['GitHub Packages', 'Both', 'All'].includes(
      answers.publishTo
    );

    let packageDir = process.cwd();
    if (!providedAnswers && answers.useMonorepo === 'Yes') {
      const packageName = answers.name.startsWith('@')
        ? answers.name.split('/')[1]
        : answers.name;
      packageDir = path.join(answers.monorepoRoot, 'packages', packageName);
    ...
    if (
      !skipFiles.includes('.env') &&
      ['GitHub Packages', 'Both', 'Custom', 'All'].includes(answers.publishTo)
    ) {
