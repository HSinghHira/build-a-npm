...
async function init(
  ...
  try {
    ...
    // Generate .npmrc if GitHub or GitHub Pages selected and token not skipped
    if (
    ...
    // Generate WEBPAGE.md for GitHub Pages if selected
    if (answers.createGitHubPages === 'Yes') {
      const webpageContent = `# ${answers.name} Documentation
...
`;
    ...
    // Generate generateIndex.js for GitHub Pages if selected
    if (answers.createGitHubPages === 'Yes') {
...
`;
    ...
    if (isGitHub) {
      if (answers.githubToken.toLowerCase() === 'na') {
        console.log(
          colorize(
            `${step++}. Set your GITHUB_TOKEN environment variable for GitHub Packages`,
            '36'
          )
        );
        console.log(
          colorize(
            "   - Create a token at https://github.com/settings/tokens with 'write:packages' and 'pages:write' scopes",
            '33'
          )
        );
      } else {
        console.log(
          colorize(
            `${step++}. Verify your GITHUB_TOKEN in .npmrc has the 'write:packages' and 'pages:write' scopes`,
            '36'
          )
        );
      }
      if (answers.createGitHubWorkflow === 'Yes') {
        console.log(
          colorize(
            `${step++}. Configure ${answers.ciProvider} secrets (NPM_TOKEN and/or GITHUB_TOKEN)`,
            '36'
          )
        );
      }
      if (answers.createGitHubPages === 'Yes') {
        console.log(
          colorize(
            `${step++}. Edit WEBPAGE.md to customize your GitHub Pages content`,
            '36'
          )
        );
        console.log(
          colorize(
            `${step++}. Run \`npm run index\` to regenerate index.html after editing WEBPAGE.md`,
            '36'
          )
        );
        console.log(
          colorize(
            `${step++}. Ensure index.html is committed to the main branch`,
            '36'
          )
        );
        console.log(
          colorize(
            `${step++}. Enable GitHub Pages in your repository settings (Settings > Pages > Source: main branch, / (root))`,
            '36'
          )
        );
      }
    ...
    console.log(
      colorize(`${step++}. Run \`npm install\` to install dependencies`, '36')
    );
    if (isWindows) {
      console.log(
        colorize(
    ...
    console.log(
      colorize(
        `${step++}. Run \`npm run publish\` to publish your package`,
        '36'
      )
    );
