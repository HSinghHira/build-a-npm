function generatePackageJson(answers) {
  const scripts = {
    ...
    git: `npm run index && git add . && git commit -m "Update to v${answers.version}" && git branch -M main && (git remote get-url origin || git remote add origin https://github.com/${answers.githubUsername}/${answers.githubRepoName}.git) && git push`,
