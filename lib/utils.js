    ...
    return {
      prompt: async questions => {
        ...
        for (const question of questions) {
          ...
          if (
            when &&
            !(typeof when === 'function' ? await when(answers) : when)
          ) {
            continue;
          }
          let resolvedMessage =
            typeof message === 'function' ? await message(answers) : message;
          if (!resolvedMessage) {
                ...
                validate: validate
                  ? async input => {
                      try {
                        ...
                        logger.error(
                          `Validation error for ${name}: ${err.message}`,
                          err.stack
                        );
            ...
            case 'list':
              answer = await prompts.select({
                ...
                choices: choices.map(c => ({
                  value: c,
                  name: typeof c === 'object' ? c.name || c : c,
                })),
                default: resolvedDefault,
              });
              break;
...
const configSchema = {
  ...
  properties: {
    ...
    createGitHubPages: { type: 'string', enum: ['Yes', 'No'] },
    ciProvider: {
      type: 'string',
...
function writeFile(filePath, content, description, dryRun) {
  if (dryRun) {
