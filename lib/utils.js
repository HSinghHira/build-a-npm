...
async function getInquirer() {
  try {
    ...
    // Create a prompt wrapper to mimic inquirer.prompt behavior
    return {
      prompt: async (questions) => {
        ...
        for (const question of questions) {
          ...
          switch (type) {
            case "input":
              answer = await prompts.input({ message, default: defaultValue, validate });
              break;
            case "list":
              answer = await prompts.select({ message, choices: choices.map(c => ({ value: c, name: c })), default: defaultValue });
              break;
            default:
