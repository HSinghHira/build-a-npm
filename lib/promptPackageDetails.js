const { getInquirer, fs, path, logger, fetchNpmPackage } = require("./utils");
...
async function promptPackageDetails(config = {}) {
  ...
  if (!inquirer || !inquirer.prompt) {
    ...
    throw new Error("Failed to initialize inquirer module. Please ensure @inquirer/prompts is installed correctly.");
  }
  ...
  const questions = [
    ...
    },
    {
      type: "input",
      name: "monorepoRoot",
      message: "Enter the monorepo root directory (relative to current directory):",
    ...
    },
    {
      ...
      choices: ["Yes", "No"],
      default: config.createGitHubWorkflow !== undefined ? (config.createGitHubWorkflow ? "Yes" : "No") : "Yes",
      when: (answers) => ["GitHub Packages", "Both"].includes(answers.publishTo),
    },
    {
      type: "list",
    ...
    },
    {
      ...
      validate: async (input, answers = {}) => {
        ...
        if (input.length > 214) return "Package name must be 214 characters or less";

        const reservedNames = ["node", "npm", "http", "https", "core", "js"];
        const namePart = input.startsWith("@") ? input.split("/")[1] : input;
        if (reservedNames.includes(namePart)) return `Package name "${namePart}" is reserved by npm`;

        if ((answers.publishTo === "npmjs" || answers.publishTo === "Both") && input.startsWith("@")) {
          if (!/^@[a-z0-9-][a-z0-9-]{0,49}\/[a-z0-9][a-z0-9-]{0,49}$/.test(input))
          ...
          if (answers.publishTo === "Both" && answers.githubUsername && input.split("/")[0] !== `@${answers.githubUsername}`)
        ...
        if (!/^[a-z0-9][a-z0-9-]{0,49}$/.test(input))
    ...
    },
    {
      type: "input",
      name: "githubToken",
      message: "Enter your GitHub Personal Access Token (GITHUB_TOKEN) or 'NA' to skip:",
      default: config && config.githubToken ? config.githubToken : undefined,
      when: (answers) => ["GitHub Packages", "Both"].includes(answers.publishTo),
      validate: (input) => {
        input = input.trim();
        if (input.toLowerCase() === "na") return true;
        if (input === "") return "GitHub token is required (or enter 'NA' to skip)";
    ...
    },
    {
      ...
      choices: ["Yes", "No"],
      default: config.useVitePress ? "Yes" : "No",
    },
    {
      type: "input",
      name: "docsDir",
      ...
      when: (answers) => answers.useVitePress === "Yes",
      validate: (input) => {
        if (input.trim() === "") return "Documentation directory is required";
        if (!/^[a-z0-9-._/]+$/.test(input))
          return "Directory name must contain only lowercase letters, numbers, hyphens, periods, or slashes";
        return true;
      },
    },
    {
      type: "input",
      name: "basePath",
      message: "Enter the base path for GitHub Pages (e.g., /repo-name/ for https://username.github.io/repo-name/):",
      default: (answers) => `/${answers.githubRepoName || answers.name}/`,
      when: (answers) => answers.useVitePress === "Yes" && answers.createGitHubRepo === "Yes",
      validate: (input) => {
        if (!input.startsWith("/") || !input.endsWith("/"))
          return "Base path must start and end with a slash (e.g., /repo-name/)";
        return true;
      },
    },
    {
    ...
    },
    {
      ...
      message: "Enter dependency version (e.g., ^4.17.21, or 'latest' to skip):",
      default: "latest",
      when: (answers) => answers.addDependency === "Yes",
    },
    {
    ...
    },
    {
      ...
      message: "Enter devDependency version (e.g., ^8.0.0, or 'latest' to skip):",
      default: "latest",
      when: (answers) => answers.addDevDependency === "Yes",
    },
    {
    ...
    },
    {
      ...
      message: "Add custom npm scripts to your project?",
      choices: ["Yes", "No"],
      default: "No",
    },
    {
      ...
      default: config && config.customScripts
  ...
  while (answers.addDependency === "Yes" && answers.addAnotherDependency === "Yes") {
  ...
  while (answers.addDevDependency === "Yes" && answers.addAnotherDevDependency === "Yes") {
  ...
  answers.version = answers.version === "Custom" ? answers.customVersion : answers.version.split(" ")[0];
  ...
  answers.customScripts = answers.addCustomScripts === "Yes" ? answers.customScripts : {};
  ...
  delete answers.addCustomScripts;
