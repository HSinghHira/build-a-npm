...
async function promptPackageDetails(config = {}) {
  ...
  const questions = [
    ...
    },
    {
      ...
      message: 'Enter the custom registry URL (e.g., https://npm.cloudsmith.io/org/repo/, or "NA" to skip):',
    ...
    },
    {
      ...
      message: 'Enter the authentication token for the custom registry (or "NA" to skip):',
    ...
    },
    {
      ...
      message: 'Enter the authentication token for Artifactory (or "NA" to skip):',
      default: config.artifactoryToken || 'NA',
      when: answers => answers.publishTo === 'All' && answers.artifactoryUrl.toLowerCase() !== 'na',
    },
    {
      type: 'input',
      name: 'nexusToken',
      ...
      when: answers => answers.publishTo === 'All' && answers.nexusUrl.toLowerCase() !== 'na',
    },
    {
      type: 'input',
      name: 'verdaccioToken',
      message: 'Enter the authentication token for Verdaccio (or "NA" to skip):',
      default: config.verdaccioToken || 'NA',
      when: answers => answers.publishTo === 'All' && answers.verdaccioUrl.toLowerCase() !== 'na',
    },
    {
      type: 'list',
      name: 'access',
      ...
      default:
        config.createGitHubWorkflow !== undefined
          ? config.createGitHubWorkflow
            ...
            : 'No'
          : 'Yes',
      when: answers => ['GitHub Packages', 'Both', 'All'].includes(answers.publishTo),
    },
    {
      type: 'list',
    ...
    },
    {
      ...
      validate: async (input, answers = {}) => {
        ...
        if (
          (['npmjs', 'Both', 'All'].includes(answers.publishTo)) &&
          input.startsWith('@')
        ) {
          if (
          ...
          if (
            (['Both', 'All'].includes(answers.publishTo)) &&
        ...
        if (['GitHub Packages', 'All'].includes(answers.publishTo) && answers.githubUsername)
