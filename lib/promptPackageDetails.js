...
async function promptPackageDetails(config = {}) {
  ...
  const questions = [
      ...
      validate: async (input, answers) => {
        ...
        if (
          ...
          try {
            ...
            return "Invalid GitHub token. Ensure it has the correct permissions (repo, pages:write for GitHub Pages) or enter 'NA' to skip.";
          }
        }
        return true;
      },
    ...
    },
    {
      ...
      when: answers => answers.publishTo !== 'GitHub Pages',
      filter: input => {
        if (!input) return input;
        return input.match(/^https?:\/\//) ? input : `https://${input}`;
      },
