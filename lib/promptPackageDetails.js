...
async function promptPackageDetails(config = {}) {
  ...
  const questions = [
    ...
    },
    {
      ...
      message: 'Publish WEBPAGE.md as documentation on GitHub Pages?',
      choices: ['Yes', 'No'],
    ...
    },
    {
      ...
      name: 'githubToken',
      message:
        "Enter your GitHub Personal Access Token with 'repo' and 'pages:write' scopes (or 'NA' to skip):",
      default: config && config.githubToken ? config.githubToken : 'NA',
      when: answers =>
        ['GitHub Packages', 'Both'].includes(answers.publishTo) ||
        answers.createGitHubPages === 'Yes',
      validate: async (input, answers) => {
          ...
          try {
            ...
            for (const scope of requiredScopes) {
              if (!tokenScopes.includes(scope)) {
                return `GitHub token must have ${scope} scope. Create a token at https://github.com/settings/tokens with ${requiredScopes.join(' and ')} permissions.`;
              }
            }
            return true;
          } catch (err) {
            logger.warn(`GitHub token validation failed: ${err.message}`);
            return "Invalid GitHub token. Ensure it has the correct permissions ('repo' for repository creation, 'pages:write' for GitHub Pages) or enter 'NA' to skip. Create a token at https://github.com/settings/tokens.";
          }
        }
        return true;
      },
