const { fs, path, logger } = require("./utils");
...
function generateVitePressFiles(answers, projectDir) {
  ...
  // Create directories
  logger.debug(`Creating directory: ${docsDir}`);
  fs.mkdirSync(docsDir, { recursive: true });
  logger.debug(`Creating directory: ${path.join(docsDir, ".vitepress")}`);
  fs.mkdirSync(path.join(docsDir, ".vitepress"), { recursive: true });
  logger.debug(`Creating directory: ${path.join(docsDir, ".vitepress", "theme")}`);
  fs.mkdirSync(path.join(docsDir, ".vitepress", "theme"), { recursive: true });
  if (answers.createGitHubRepo === "Yes") {
    logger.debug(`Creating directory: ${githubWorkflowDir}`);
