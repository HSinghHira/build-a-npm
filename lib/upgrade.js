  ...
  try {
    ...
    const {
      ...
      },
      {
        ...
        default: !!existingPackageJson.types,
      ...
      },
      {
        ...
        default: !!existingPackageJson.devDependencies?.eslint,
        when: (answers) => answers.confirm,
      },
      {
        type: "confirm",
        ...
        default: !!existingPackageJson.devDependencies?.prettier,
        when: (answers) => answers.confirm,
      },
      {
        type: "list",
    ...
    const answers = {
      publishTo,
      createGitHubWorkflow:
        createGitHubWorkflow !== undefined ? createGitHubWorkflow : false,
      useTypeScript,
      useESLint,
      usePrettier,
      access,
    ...
    if (answers.useTypeScript && !fs.existsSync("tsconfig.json")) {
    ...
    if (answers.useESLint && !fs.existsSync(".eslintrc.json")) {
      const eslintConfigContent = generateEslintConfig(answers.useTypeScript);
      if (verbose) {
    ...
    if (answers.usePrettier && !fs.existsSync(".prettierrc")) {
    ...
    if (!fs.existsSync(".gitignore")) {
      const gitignoreContent = generateGitignore(answers.useTypeScript);
      if (verbose) {
