...
async function upgrade(packageVersion, verbose, dryRun) {
  ...
  try {
    ...
    const answers = {
      ...
      githubUsername:
        githubUsername || // Use prompted value if provided
        (isGitHub
          ? existingPackageJson.name.split('/')[0]?.replace('@', '') ||
          ...
          : 'sampleuser'),
      githubRepoName:
        githubRepoName || // Use prompted value if provided
        existingPackageJson.repository?.url
          ?.split('/')
          .slice(-1)[0]
          ?.replace('.git', '') ||
    ...
    // Update package.json with repository if GitHub Pages is selected
    if (
    ...
    // Generate WEBPAGE.md for GitHub Pages if selected and not present
    if (answers.createGitHubPages === 'Yes' && !fs.existsSync('WEBPAGE.md')) {
...
`;
    ...
    if (answers.createGitHubPages === 'Yes' && fs.existsSync('WEBPAGE.md')) {
      try {
        ...
        console.log(
          colorize(
            "⚠️ Skipped generating index.html: 'marked' module not found. Install it with `npm install marked`.",
            '33'
          )
        );
      }
    ...
    if (
      ...
      try {
        ...
        console.log(
          colorize(
            "⚠️ Skipped generating GitHub Pages workflow: 'marked' module not found. Install it with `npm install marked`.",
            '33'
          )
        );
      }
