...
function generateGitHubPagesIndex(content) {
  let title = 'Documentation'; // Fallback title
  try {
    const packageJson = JSON.parse(fs.readFileSync('package.json', 'utf-8'));
    title = `${packageJson.name}`;
  } catch (err) {
    console.warn(
      '⚠️ Could not read package.json, using default title:',
      err.message
    );
  }
...
<body>
  ...
  <div class="container">
    ...
    <main>
      ...
      <footer>
        <p>&copy; 2025 — ${title}</p>
        <p>Documentation automatically generated from README.md</p>
      </footer>
    </main>
  </div>
   ...
   <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Wrap tables in containers for better responsive handling
