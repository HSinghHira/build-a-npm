...
async function init(noGit, useSample, packageVersion, verbose, configPath) {
  try {
    ...
    if (!fs.existsSync(answers.useTypeScript ? "src/index.ts" : "index.js")) {
      const indexContent = generateIndexFile(answers.useTypeScript);
      ...
      const indexPath = answers.useTypeScript ? "src/index.ts" : "index.js";
      if (verbose) {
    ...
    if (["GitHub Packages", "Both"].includes(answers.publishTo)) {
      ...
      if (answers.createGitHubWorkflow) {
        console.log(
          colorize(
    ...
    console.log(
      colorize(
        "4. Add your package code to " +
          (answers.useTypeScript ? "src/index.ts" : "index.js"),
        "36"
      )
    );
    console.log(
